...
export async function GET(
  ...
  try {
    ...
    if (!response.ok) {
      console.error(`Error fetching album tracks data for ID ${albumId}: ${response.status} ${await response.text()}`);
      return NextResponse.json({ error: 'Failed to fetch album tracks data' }, { status: response.status });
    }
